...
const StudentDashboard = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const studentCourses = await system.getStudentCourses();
  ...
  const fetchActiveSessions = async () => {
    try {
      const sessions = await system.getActiveSessions();
      const user = system.getCurrentUser();
  ...
  const loadMonthlyReport = async () => {
    ...
    try {
      ...
      const attendanceRecords = await system.getAttendanceByCourse(selectedReportCourse);
      let userAttendance = attendanceRecords.filter(record => record.studentId === currentUser.id);
