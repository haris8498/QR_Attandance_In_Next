...
const Admin = () => {
    ...
    const loadInitialData = async () => {
        try {
            // Load all data from localStorage via system
            const students = await system.getAllStudents();
            const teachersArr = await system.getAllTeachers();
            const coursesList = await system.getAllCourses();
            const attendanceList = await system.getAllAttendance();
    ...
    const addStudent = async () => {
        ...
        try {
            const result = await system.signup(studentRollNo + '@student.com', 'student123', studentName, 'student', studentRollNo);
            ...
            alert(`Student added successfully!\nUsername: ${studentRollNo}@student.com\nPassword: student123`);
        } catch (error) {
            alert(error.message);
        }
    };
    ...
    const removeStudent = async (username) => {
        if (confirm('Are you sure you want to remove this student?')) {
            try {
                await system.deleteStudent(username);
                await loadInitialData();
    ...
    const removeCourse = async (code) => {
        ...
        try {
            await system.deleteCourse(code);
            await loadInitialData();
    ...
    const addTeacher = async () => {
        ...
        try {
            const result = await system.addTeacher(teacherName, teacherUsername);
    ...
    const removeTeacher = async (username) => {
        if (confirm('Are you sure you want to remove this teacher?')) {
            try {
                await system.deleteTeacher(username);
                await loadInitialData();
    ...
    const assignCoursesToTeacher = async () => {
        ...
        try {
            ...
            await system.assignCoursesToTeacher(selectedTeacher, allCourses);
            await loadInitialData();
            setTeacherCourses([]);
            alert(`✅ Courses assigned successfully to ${teacher.name}!`);
        } catch (err) {
    ...
    const removeCourseFromTeacher = async (teacherUsername, courseCode) => {
        ...
        try {
            ...
            await system.updateUserCourses(teacherUsername, newCourses);
            await loadInitialData();
    ...
    const addStudentsToCourse = async () => {
        ...
        try {
            await system.assignStudentsToCourse(selectedCourse, selectedStudents);
            await loadInitialData();
            setSelectedStudents([]);
    ...
    const removeStudentFromCourse = async (studentUsername, courseCode) => {
        ...
        try {
            ...
            await system.updateUserCourses(studentUsername, newCourses);
            await loadInitialData();
    ...
    const generateComprehensivePDF = async () => {
        ...
        try {
            ...
            const enrolledStudents = await system.getStudentsByCourse(reportCourse);
            const sessionDates = [...new Set(attendanceRecords.map(r => r.date))].sort();
                ...
                <body>
                    ...
                    <div class="summary">
                        ...
                        <div class="stats">
                            ...
                            <div class="stat-box">
                                <strong>${attendanceRecords.filter(r => r.status === 'present').length}</strong><br>Present Records
                            </div>
                        </div>
                    </div>
