...
const Teacher = () => {
    ...
    const loadInitialData = async () => {
        try {
            ...
            const teacherCourses = await system.getTeacherCourses();
            setCourses(teacherCourses);
            
            if (!teacherCourses.length) {
                setCoursesMessage('No courses assigned to this teacher');
            } else {
                setCoursesMessage('');
            }
            ...
            const attendanceRecords = await system.getAllAttendance();
    ...
    const checkExistingQR = async () => {
        try {
            const sessions = await system.checkGlobalSession();
            
            if (sessions && sessions.length > 0) {
                const currentUser = system.getCurrentUser();
                const mySession = sessions.find(s => s.teacherId === currentUser.id);
                
                if (mySession) {
                    const qrData = {
                        courseId: mySession.courseId,
                        teacherId: currentUser.id,
                        teacherName: currentUser.name,
                        timestamp: Date.now(),
                        expiry: mySession.expiresAt
                    };
                    const qrString = JSON.stringify(qrData);
                    setCurrentQR(qrString);
                    setSelectedCourse(mySession.courseId);
                    if (mySession.expiresAt) startQRTimer(mySession.expiresAt);
    ...
    const fetchNotifications = async () => {
        try {
            const data = await system.getNotifications();
    ...
    const fetchUnreadCount = async () => {
        try {
            const data = await system.getUnreadNotifications();
    ...
    const markNotificationRead = async (id) => {
        try {
            await system.markNotificationRead(id);
            fetchNotifications();
            fetchUnreadCount();
        } catch (err) {
    ...
    const markAllNotificationsRead = async () => {
        try {
            await system.markAllNotificationsRead();
            fetchNotifications();
            fetchUnreadCount();
        } catch (err) {
    ...
    const generateQRCode = async () => {
        ...
        try {
            ...
            if (sessions && sessions.length > 0) {
                alert(`❌ Another session is currently active. Please wait before creating a new session.`);
                return;
            }
            
            // Check if a session already exists for today for this specific course
            const attendanceRecords = await system.getAttendanceByCourse(selectedCourse);
    ...
    const markNoClass = async () => {
        ...
        if (confirm(`Mark "No Class" for ${getCourseName(selectedCourse)} today? This will show "No Class Today" to students.`)) {
            ...
            // Persist no-class flag to localStorage
            (async () => {
                try {
                    await system.markNoClass(attendanceDate, selectedCourse);
    ...
    const loadAttendanceReport = async () => {
        ...
        try {
            ...
            const attendanceRecords = await system.getAttendanceByCourse(reportCourse);
            const dateRecords = attendanceRecords.filter(r => r.date === attendanceDate);
            ...
            const enrolledStudents = await system.getStudentsByCourse(reportCourse);
    ...
    const downloadSessionsPDF = async () => {
        ...
        try {
            ...
            const attendanceRecords = await system.getAttendanceByCourse(reportCourse);
            const sessions = [...new Set(attendanceRecords.map(r => r.date))].sort();
    ...
    const viewDetailedReport = async () => {
        ...
        try {
            ...
            const percentage = enrolledStudents.length > 0 ? ((presentCount / enrolledStudents.length) * 100).toFixed(1) : 0;
    ...
    const loadStudentsForCourse = async (courseCode, selectedDate = attendanceDate) => {
        ...
        try {
            const enrolledStudents = await system.getStudentsByCourse(courseCode);
            setStudents(enrolledStudents);
            
            if (selectedDate) {
                const attendanceRecords = await system.getAttendanceByCourse(courseCode);
    ...
    const markManualAttendance = async (studentId, status) => {
        ...
        try {
            await system.markManualAttendance(manualAttendanceCourse, [studentId], attendanceDate);
    ...
    const saveAllAttendance = async () => {
        ...
        try {
            const presentStudents = students
                .filter(student => (attendanceStatuses[student.username] || 'absent') === 'present')
                .map(student => student.username);
            
            await system.markManualAttendance(manualAttendanceCourse, presentStudents, attendanceDate);
